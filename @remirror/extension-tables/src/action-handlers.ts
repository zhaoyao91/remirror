...
import { defaultTableSelection, pluginKey } from './plugins/main';
...
export const handleSetTableRef = (state: EditorState, tableRef?: HTMLElement) => (
  pluginState: TablePluginState,
  dispatch: Dispatch,
): TablePluginState => {
  const nextPluginState = {
    ...pluginState,
    tableRef,
    tableFloatingToolbarTarget: closestElement(tableRef, `.${ClassName.TABLE_NODE_WRAPPER}`) || undefined,
...
export const handleClearSelection = (pluginState: TablePluginState, dispatch: Dispatch): TablePluginState => {
  const { decorationSet } = pluginState;
  const nextPluginState = {
    ...pluginState,
    ...defaultTableSelection,
    decorationSet: decorationSet.remove(findControlsHoverDecoration(decorationSet)),
  };
  dispatch(pluginKey, nextPluginState);
  return nextPluginState;
};
...
export const handleDocOrSelectionChanged = (tr: Transaction) => (
  ...
  const hoverDecoration = findControlsHoverDecoration(pluginState.decorationSet);
...
export const handleShowInsertColumnButton = (insertColumnButtonIndex?: number) => (
  pluginState: TablePluginState,
  dispatch: Dispatch,
): TablePluginState => {
  const nextPluginState = {
