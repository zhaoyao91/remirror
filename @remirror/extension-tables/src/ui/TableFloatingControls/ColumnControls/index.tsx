...
export default class ColumnControls extends Component<Props, any> {
  public shouldComponentUpdate(nextProps: Props) {
    ...
    if (nextProps.tableRef) {
      ...
      const shouldUpdate = Math.abs(controls.offsetWidth - nextProps.tableRef.offsetWidth) > 1;
      if (shouldUpdate) {
        return true;
      }
    }
  ...
  public render() {
    ...
    const deleteBtnParams = getColumnDeleteButtonParams(columnsWidths, selection);

    return (
      <div className={ClassName.COLUMN_CONTROLS}>
        <div className={ClassName.COLUMN_CONTROLS_INNER}>
          <>
            {columnsParams.map(({ startIndex, endIndex, width }: ColumnParams) => (
              <div
                className={`${ClassName.COLUMN_CONTROLS_BUTTON_WRAP} ${getColumnClassNames(
                  startIndex,
                  selection,
                  hoveredColumns,
                  isInDanger,
              ...
              >
                ...
                >
                  {!isCellSelection(selection) && (
                    <>
                      <div className={ClassName.CONTROLS_BUTTON_OVERLAY} data-index={startIndex} />
                      <div className={ClassName.CONTROLS_BUTTON_OVERLAY} data-index={endIndex} />
                    </>
                  )}
                </button>
                {isColumnInsertButtonVisible(endIndex, selection) && (
                  <InsertButton
                    ...
                    tableRef={tableRef}
                    index={endIndex}
                    showInsertButton={!isResizing && insertColumnButtonIndex === endIndex}
            ...
            {isColumnDeleteButtonVisible(selection) && deleteBtnParams && (
              <DeleteButton
                ...
                onMouseEnter={() => this.hoverColumns(deleteBtnParams.indexes, true)}
