...
import { getPluginState } from '../plugins/main';
import { ResizeState, scaleTable } from '../plugins/table-resizing';
...
class TableComponent extends React.Component<ComponentProps, TableState> {
  ...
  public componentDidMount() {
    ...
    if (allowColumnResizing) {
      const { view, node, containerWidth, getPos, dynamicTextSizing } = this.props;

      if (node.attrs.__autoSize === false) {
        const parentWidth = this.getParentNodeWidth(view.state, containerWidth.width);
  ...
  public render() {
    const { view, node, pluginState, tableResizingPluginState, width } = this.props;
    ...
    const rowControls = [
      <div
        key={0}
        className={`${ClassName.ROW_CONTROLS_WRAPPER} ${scroll > 0 ? ClassName.TABLE_LEFT_SHADOW : ''}`}
      >
  ...
  private handleTableResizing = (prevProps: ComponentProps) => {
    const { view, node, getPos, containerWidth, dynamicTextSizing } = this.props;
    ...
    const layoutChanged =
      prevAttrs.layout !== currentAttrs.layout && prevAttrs.__autoSize === currentAttrs.__autoSize;
  ...
  private handleAutoSize = () => {
    if (this.table) {
      const { view, node, getPos, dynamicTextSizing = false, containerWidth } = this.props;
