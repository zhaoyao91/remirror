...
import { updateRightShadow } from '../../node-views/TableComponent';
...
import { getDefaultLayoutMaxWidth, getLayoutSize, tableLayoutToSize } from './utils';

export function updateColumnWidth(view: EditorView, cell: number, movedWidth: number, resizer: Resizer) {
  const $cell = view.state.doc.resolve(cell);
  const table = $cell.node(-1);
  const map = TableMap.get(table);
  const start = $cell.start(-1);
  const col = map.colCount($cell.pos - start) + ($cell.nodeAfter ? $cell.nodeAfter.attrs.colspan : 1) - 1;
...
export function applyColumnWidths(view: EditorView, state: ResizeState, table: PMNode, start: number) {
  ...
  for (let i = 0; i < state.cols.length; i++) {
    ...
    for (let row = 0; row < map.height; row++) {
      ...
      const colwidth = attrs.colwidth ? attrs.colwidth.slice() : Array.from({ length: attrs.colspan }, _ => 0);
...
export function handleBreakoutContent(
  ...
  const amount = addContainerLeftRightPadding(minWidth - elem.offsetWidth, cellStyle);
...
export function resizeColumn(view: EditorView, cell: number, width: number, resizer: Resizer) {
  const $cell = view.state.doc.resolve(cell);
  const table = $cell.node(-1);
  const start = $cell.start(-1);
...
export const updateResizeHandle = (view: EditorView) => {
  ...
  const tableWrapperRef = findDomRefAtPos(tablePos, view.domAtPos.bind(view)) as HTMLDivElement;
  ...
  if (tableRef && resizeHandleRef) {
    const cellRef = findDomRefAtPos(activeHandle, view.domAtPos.bind(view)) as HTMLTableCellElement;
    const tableActive = closestElement(tableRef, `.${ClassName.WITH_CONTROLS}`);
    resizeHandleRef.style.height = `${
      tableActive ? tableRef.offsetHeight + akEditorTableToolbarSize : tableRef.offsetHeight
    }px`;

    resizeHandleRef.style.left = `${cellRef.offsetLeft + cellRef.offsetWidth}px`;
  }
};
...
export const updateControls = (state: EditorState) => {
  ...
  const columnControls: any = wrapper.querySelectorAll(`.${ClassName.COLUMN_CONTROLS_BUTTON_WRAP}`);
  const rows = tableRef.querySelectorAll('tr');
  const rowControls: any = wrapper.parentElement.querySelectorAll(`.${ClassName.ROW_CONTROLS_BUTTON_WRAP}`);
...
function scale(
  ...
  const maxSize = getLayoutSize(node.attrs.layout, containerWidth, dynamicTextSizing);
