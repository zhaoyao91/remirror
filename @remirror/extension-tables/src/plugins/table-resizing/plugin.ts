...
import { resizeColumn, updateColumnWidth, updateControls, updateResizeHandle } from './actions';
import Resizer from './resizer/resizer';

import { currentColWidth, domCellAround, edgeCell, getLayoutSize, pointsAtCell } from './utils';

import { ColumnResizingPlugin, TableCssClassName as ClassName } from '../../../types';
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, pluginState: ResizeState, prevState, state) {
        ...
        if (
          (newPluginState && pluginState.activeHandle !== newPluginState.activeHandle) ||
      ...
      handleDOMEvents: {
        ...
        mousedown(view, event) {
          ...
          if (activeHandle > -1 && !dragging) {
            handleMouseDown(view, event as MouseEvent, cellMinWidth, dynamicTextSizing);
            updateResizeHandle(view);
            return true;
          }
...
export class ResizeState {
  constructor(public activeHandle: number, public dragging: { startX: number; startWidth: number } | null) {
    return Object.freeze(this);
  }

  public apply(tr: Transaction, state: EditorState) {
    const action = tr.getMeta(pluginKey);
    const { editorDisabled } = editorDisabledPluginKey.getState(state) as EditorDisabledPluginState;
...
function handleMouseDown(
  ...
  let resizeHandleRef: HTMLDivElement | null = createResizeHandle(dom as HTMLTableElement);

  const containerWidth = widthPluginKey.getState(view.state).width;
  const resizer = Resizer.fromDOM(view, dom, {
    minWidth: cellMinWidth,
    maxSize: getLayoutSize(dom.getAttribute('data-layout') as TableLayout, containerWidth, dynamicTextSizing),
    node: $cell.node(-1),
    start,
  });
  ...
  const width = currentColWidth(view, activeHandle, cell!.attrs as CellAttributes);
  view.dispatch(
