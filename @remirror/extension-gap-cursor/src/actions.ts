...
import { isTextBlockNearPosition } from './utils';

const inMiddle;
export const arrow = (dir: Direction): CommandFunction => (state, dispatch, view) => {
  ...
  if (isTextSelection(selection)) {
    ...
    if (movingToNewTextBlock({ dir, state, $pos })) {
      return false;
    }
...
interface MovingToNewTextBlockParams {
  dir: Direction;
  state: EditorState;
  $pos: ResolvedPos;
}

const movingToNewTextBlock = ({ dir, state, $pos }: MovingToNewTextBlockParams) => {
  const { doc, schema } = state;
  return (
    (dir === Direction.UP && !atDocStart(state) && isTextBlockNearPosition(doc, schema, $pos, -1)) ||
    (dir === Direction.DOWN && (atDocEnd(state) || isTextBlockNearPosition(doc, state.schema, $pos, 1)))
  );
};
